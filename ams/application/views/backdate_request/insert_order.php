      ...
      <form id="form">
            <fieldset id="personal">
                  ...
                  <div style="float: left; width: 400px;">
                        ...
                        <br>
                        <label for="benefit">Alasan : </label> 
                        <textarea name="txtReason" id="txtReason" style="height: 80px; width: 250px;"></textarea> <span class="error" id="errReason"></span>
                  </div>
                  <div style="float: left; width: 400px;">
                        ...
                        <tbody id="listpaket">
                              ...
                              foreach ($all_detail as $detail):
                                    ...
                                    <tr class='remove'>
                                          ...
                                          <td align="center">
                                                <?php if ($detail["backdate"] == "Y"): ?>
                                                      <img src="<?php echo image_path("checked.png"); ?>" title="Was Request" width="16" height="16" />
                                                <?php else: ?>
                                                      <input type="checkbox" name="chkRequest" value="<?php echo $detail["id"]; ?>" />
                                                <?php endif; ?>
            ...
            <div align="center">
                  <input id="button1" type="button" value="Simpan" onclick="ajaxChange('backdate_request', 'insert', '<?php echo site_url("backdate_request/insert"); ?>', '<?php echo site_url("backdate_request/content"); ?>', '<?php echo site_url("backdate_request/insert_page"); ?>')" /> 
                  <input id="button2" type="button" value="Reset" onclick="loadForm('<?php echo site_url("backdate_request/insert_page"); ?>')" />
            </div>
      </form>
      ...
      <script type="text/javascript">
            $(document).ready(function() {
                  ...
                  $("input[name=rdbPacket]").click(function() {
                        if ($(this).val() == "Y")
                              loadForm('<?php echo site_url("backdate_request/insert_page"); ?>')
                        else
                              loadForm('<?php echo site_url("backdate_request/search_page"); ?>')
                  });
                                    
                  $("input[name=chkRequest]").click(function() {
                        if ($(this).is(":checked")) {
                              $(this).parent().prev().prev().children("#new_period").html("<input name='txtStartDate' class='txtStartDate' type='text' style='width:80px;' onmousedown='$(this).datepicker({dateFormat: \"yy-mm-dd\"});' />"+
                                    " - "+
                                    "<input name='txtEndDate' class='txtEndDate' type='text' style='width:80px;' onmousedown='$(this).datepicker({dateFormat: \"yy-mm-dd\"});' />");
                        } else
                              $(this).parent().prev().prev().children("#new_period").empty();
                  });
            });
      </script>
<?php endif; ?>