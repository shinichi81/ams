...
class Report_Closing_Ae extends CI_Controller {
      /* Controller ini untuk menampilkan halaman order
       * Lokasi: ./application/controllers/Report_Closing_Ae.php 
       */
      ...
      public function __construct() {
            ...
            $this->_access = get_access("REPORT_CLOSING");
      ...
      public function index() {
            ...
            $data["all_ae"] = $allAe;
            $data["page"] = "report/closing/index";
            $data["menu"] = "report";
...
//            $yearMonth = $year . "-" . $month;
            ...
            $allData = $this->Report_Model->getAllClosing($startLimit, $endLimit, $ae, $startDate, $endDate);
            foreach ($allData as $key => $result) {
                  $totalPaket = $this->Report_Model->getTotalPaketClosingRevisi($result->ae_id, $result->no_paket, $startDate, $endDate);
                  $allData[$key]->{"total_paket"} = $totalPaket;
            }

            // untuk mendapatkan total page
            $total = $this->Report_Model->getTotalClosing($ae, $startDate, $endDate);
...
//            $data["yearmonth"] = $yearMonth;
            ...
            $this->load->view("report/closing/content", $data);
      }

      public function excel($ae, $startDate, $endDate) {
            $arrData = array();
            $aeName = "-";
            ...
            $allData = $this->Report_Model->getAllClosing(null, null, $ae, $startDate, $endDate);

            foreach ($allData as $key => $value) {
                  //$marketing = $this->Report_Model->getName($value->marketing_id);
                  $totalPaket = $this->Report_Model->getTotalPaketClosingRevisi($value->ae_id, $value->no_paket, $startDate, $endDate);
...
//            $data["yearmonth"] = $yearMonth;
            ...
            $this->load->view("report/closing/excel", $data);
      }
