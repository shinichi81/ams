...
class Order extends CI_Controller {
    ...
    public function insert() {
        ...
        $miscInfoEvent = $arrParam[17];
        $miscInfoProductionCost = $arrParam[18];
        $cpmQuota = $arrParam[19];
        $industrycat_id = $arrParam[20];
		$price_deal = $arrParam[21];
		$price_position = $arrParam[22];
...
//                              if ($allow_override == "N") {
        ...
        if (($packet_type == "N" and empty($no)) or empty($agency_id) or empty($client_id) or empty($diskon) or empty($selectAds) or $tempIdConflict <> "" or $tempIdInUse <> "" or $tempIdCpmQuotaEmpty <> "" or $tempIdCpmQuota <> "" or $tempIdDateWrong <> "" or !$validNo) {
            $data["status"] = false;
            $data["error"] = array();
            ...
            try {
                $insert = $this->Order_Model->insertOrderPaket($no_paket, $agency_id, $client_id, $budget, $diskon, $benefit, $isRestrict, $industry_id, $no, $miscInfo, $miscInfoEvent, $miscInfoProductionCost, $industrycat_id);
                if ($insert !== true)
                ...
                for ($n = 0; $n < count($selectAds); $n++) {
                    ...
                    $cpm_quota = str_replace(".", "", $cpmQuota[$n]["value"]);
					$per_price_deal = $price_deal[$n]["value"];
					$per_price_position = $price_position[$n]["value"];

                    $insert = $this->Order_Model->insertOrderPaketAds($no_paket, $ads_id, $kanal_id, $product_group_id, $position_id, $start_date, $end_date, $misc_info_paket, $cpm_quota, $per_price_deal, $per_price_position);
                    if ($insert !== true)
                        throw new Exception($insert);
                }
    ...
    public function update() {
		...
		$price_deal = $arrParam[22];
		$price_position = $arrParam[23];
        $date = true;
        $tempIdInUse = "";
        $tempIdConflict = "";
        $tempIdDateWrong = "";
...
//                              if ($allow_override == "N") {
        ...
        if (empty($no_paket) or empty($agency_id) or empty($client_id) or empty($diskon) or empty($selectAds) or $tempIdConflict <> "" or $tempIdInUse <> "" or $tempIdCpmQuotaEmpty <> "" or $tempIdCpmQuota <> "" or $tempIdDateWrong <> "") {
            ...
            $this->Transaction_Model->transaction_start();
            try {
                $update = $this->Order_Model->updateOrderPaket($no_paket, $agency_id, $client_id, $budget, $diskon, $benefit, $isRestrict, $industry_id, $miscInfo, $miscInfoEvent, $miscInfoProductionCost, $industrycat_id);
                if ($update !== true)
                ...
                for ($n = 0; $n < count($selectAds); $n++) {
                    ...
                    $update = $this->Order_Model->insertOrderPaketAds($no_paket, $ads_id, $kanal_id, $product_group_id, $position_id, $start_date, $end_date, $misc_info_paket, $cpm_quota, $per_price_deal, $per_price_position);
                    if ($update !== true)
                        throw new Exception($update);
                }
            } catch (Exception $e) {
