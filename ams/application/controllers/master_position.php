...
class Master_Position extends CI_Controller {
      ...
      public function content() {
            ...
            $allData = $this->Position_Model->getAll($startLimit, $endLimit);

            // untuk mendapatkan total page
            $total = $this->Position_Model->getTotal();
...
//            $cpm_quota = ($allow_override == "Y") ? str_replace(".", "", $arrParam[3]) : 0;
            ...
            if (empty($name)) {
                  ...
                  $insert = $this->Position_Model->insert($name, $misc_info, $price/*, $allow_override, $cpm_quota*/);
...
//            $cpm_quota = ($allow_override == "Y") ? str_replace(".", "", $arrParam[4]) : 0;
            ...
            if (empty($name)) {
                  ...
                  $update = $this->Position_Model->update($id, $name, $misc_info, $price/*, $allow_override, $cpm_quota*/);
