...
function ajaxChange(obj, todo, urlChange, urlContent, urlInsert, id) {
      ...
      if (obj == "client") {
            ...
            if (todo == "insert") {
                  ...
                  var arrParam = new Array(txtName, txtKeterangan, price/*, txtOverride, txtCpmQuota*/);
            } else if (todo == "update") {
                  var hdId = $("#hdId").val();
                  var txtName = $("#txtName").val();
                  var txtKeterangan = $("#txtKeterangan").val();
                  ...
                  //                  var txtOverride = $("input[name=rdbOverride]:checked").val();
                  //                  var txtCpmQuota = $("#txtCpmQuota").val();
                  var arrParam = new Array(hdId, txtName, txtKeterangan, price/*, txtOverride, txtCpmQuota*/);
            } else if (todo == "delete") {
                  var arrParam = new Array(id);
            }
            ...
            if (todo == "insert") {
                  ...
                  var price_deal = ($("input[name=price_deal]").length > 0) ? $("input[name=price_deal]").serializeArray() : "";
                  var price_position = ($("input[name=price_position]").length > 0) ? $("input[name=price_position]").serializeArray() : "";
...
function do_process(obj, todo, urlChange, urlContent, urlInsert, arrParam) {
      ...
      function(data) {
            ...
            if (data == null) {
                  show_dialog_confirmation("Session habis. Silahkan login kembali!");
      ...
      jqxhr.error(function() {
            $.fancybox({
                  "content" : "Error occurred. Try again2!"
            });
            hide_loading();
      });
...
function loadContent(url, obj) {
      ...
      function(data) {
            $("#data").hide();
            if (data == "")
                  $("#data").html("<p>Session habis. Silahkan login kembali!</p>");
            else 
                  $("#data").html(data);
            $("#data").fadeIn("slow");
      }
      ...
      jqxhr.error(function() {
            $.fancybox({
                  "content" : "Error occurred. Try again3!"
            });
            $("#data").hide();
      });
...
function loadShow(url, id, kanal_id, product_group_id, position_id, start_date, end_date) {
      ...
      function(data) {
            if (data == "")
                  data = "Session habis. Silahkan login kembali!";
...
function loadForm(url, id) {
      ...
      function(data) {
            $(".box_form").hide();
            if (data == "")
                  $(".box_form").html("<p>Session habis. Silahkan login kembali!</p>");
            else
                  $(".box_form").html(data);
      ...
      jqxhr.error(function() {
            $.fancybox({
                  "content" : "Error occurred. Try again12!"
            });
            $(".box_form").hide();
      });
